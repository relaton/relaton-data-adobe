...
module AdobeFetcher
  ...
  # relaton-index.
  #
  # No type-specific logic — works uniformly for tech notes and named
  # publications. The index mirrors the data/ directory exactly on
  # every run.
  #
  ...
  #     index_v2_file is given), parsed via pubid v2's Adobe support
  module Indexer
    module_function
    ...
    def build(data_dir:, index_file:, index_v2_file: nil)
      ...
      Dir[File.join(data_dir, "*.yaml")].sort.each do |f|
        item = Relaton::Adobe::Item.from_yaml(File.read(f, encoding: "UTF-8"))
        ...
        # index-v2 parses the structural id (item.id) rather than the
        # citation form (docid.content). For Adobe Publications the
        # citation uses the title ("Adobe Publication <title>") which
        # isn't slug-parseable; the id field is always slug/ATN-based
        # and round-trips cleanly through Pubid::Adobe.
    ...
    def clean_index(file:, pubid_class: nil)
      idx = Relaton::Index.find_or_create :Adobe, file: file, pubid_class: pubid_class
      idx.remove_all
      idx
    end
    ...
    def resolve_pubid_class
      begin
        # `require "pubid"` registers `Pubid::Adobe` via autoload in
        # lib/pubid.rb; referencing the constant then loads the flavor.
        require "pubid"
        Pubid::Adobe::Identifier
