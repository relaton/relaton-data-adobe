...
module AdobeFetcher
  # HTTP seam. Fetchers call +Http.backend.get(url)+; the default
  # backend is +NetHttp+ (real network). Tests install a +Fake+ with a
  # fixture table.
  #
  # Two adapters justify the seam: real network in prod, fixture-driven
  # in tests. Redirect policy, timeouts, and error classes live in
  # +NetHttp+ so callers don't reimplement them.
  module Http
    ...
    # Production adapter. Follows up to +redirects+ hops, raises on
    # 4xx/5xx, raises on read/open timeout.
    class NetHttp
      ...
      def fetch_with_redirects(uri, max, read_timeout, open_timeout, depth, headers)
        raise AdobeFetcher::Http::TooManyRedirects, uri.to_s if depth >= max
        ...
        when Net::HTTPRedirection
          ...
          raise AdobeFetcher::Http::BadStatus, "HTTP #{res.code} for #{uri}"
        end
      rescue Net::ReadTimeout, Net::OpenTimeout
        raise AdobeFetcher::Http::Timeout, uri.to_s
      end
    ...
    # +table+ maps URL strings to body strings (or a Proc called with
    # the URL).
    class Fake
      ...
      def get(url, **_)
        entry = @table[url]
        return entry.call(url) if entry.is_a?(Proc)
