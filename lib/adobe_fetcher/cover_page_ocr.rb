...
module AdobeFetcher
  # GLM-OCR wrapper for Adobe PDFs whose first page has no extractable
  # text layer. Mirrors the cover_page_ocr.rb approach from
  # relaton-data-iala, which itself mirrors relaton-data-oiml.
  #
  # The fetcher prefers +pdftotext+; this is the fallback. Reads the
  # API key from +~/.zai-api-key+ or +ENV["Z_AI_API_KEY"]+. Never
  # hardcodes the key, never commits it.
  class CoverPageOcr
    ...
    # docid's filename stem (e.g. "atn5014") so the cache entry is
    # human-browsable: pdfs/ocr-cache/atn5014.md.
